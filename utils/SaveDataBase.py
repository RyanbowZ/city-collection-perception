...
class SaveForm2:
    ...
    def add_form(self, form):
        # 使用 cursor() 方法创建一个 dict 格式的游标对象 cursor
        cursor = self.db.cursor()
        sql = "INSERT INTO user_info2(judge_num,user_safe,user_boring,user_beauty,user_lively,user_depressing) VALUES (%s,%s,%s,%s,%s,%s)"
        ...
        user_beauty = form[3]
        user_lively = form[4]
        user_depressing = form[5]

        values = (judge_num,user_safe,user_boring,user_beauty,user_lively,user_depressing)
        # values = (user_id,judge_num,user_gender,user_age,user_profession,user_earthquake,user_living,user_travel,user_advice)
        cursor.execute(sql, values)
        self.db.commit()
        # 使用 fetchone() 方法获取单条数据.
